...
class BlackHoleBrowser(object):
    ...
    def __init__(self, blackhole):
        """
        Creates the main GUI, the browser.
        """
        BlackHoleBrowser.blackHole = blackhole
        if not BlackHoleBrowser.blackHole:
            raise BlackHoleMissing()
        self.palette = [
            ('body', 'black', 'light gray'),  # Normal Text
            ('focus', 'light green', 'dark blue', 'standout'),  # Focus
        ...
        self.footer_text = [
            ...
            ('key', "q"), "  ",
            ('footer_msg', "\nChat:"), " ",
            ('key', "c" if BlackHoleBrowser.blackHole.settings.chat_enabled else _("Disabled")), "  ",
            ('footer_msg', "By:"), " ",
            ('key', "%s [%s]" % ("Rebagliati, Nicolas", "nicolas.rebagliati@aenima-x.com.ar")), ]
        self.quit = False
        self.listbox = urwid.ListBox(EnvironmentWalker(BlackHoleBrowser.blackHole.data.user))
    ...
    def run(self):
        ...
        try:
            ...
            while 1:
                ...
                self.header_text = [
                    ('footer_msg', "BlackHole (v%s)" % BlackHoleBrowser.blackHole.version), " ",
                    ('footer_msg', _("User:")), " ",
    ...
    def validate(self, widget):
        if not widget.error:
            ...
            if proceedSession:
                user = widget.hostConnectionObject.getConnectionUser(BlackHoleBrowser.blackHole.data.user)
                pk = BlackHoleBrowser.blackHole.getPrivateKey(user, environment)
                if pk:
                    widget.pk = pk
    ...
    def isEnvironmentAllowed(self, environment):
        if BlackHoleBrowser.blackHole.data.user.allowedByEnvironments.all():
            if environment not in BlackHoleBrowser.blackHole.data.user.allowedByEnvironments.all():
    ...
    def startSSH(self, widget):
        ...
        try:
            newClient = SecureShellClient(widget, self.size)
    ...
    def startChat(self):
        try:
            chatGui = chatGUI(BlackHoleBrowser.blackHole.data.user)
            chatGui.main()
        except Exception as e:
    ...
    * environments: environment object
    ...
    def setHosts(self, user):
        ...
        for hostConnection in user.profile.hosts.filter(host__environment=self.environmentObject).order_by('host__name'):
            if hostConnection.host.name not in self.widgets:
...
class EnvironmentTree(TreeWidget):
    def __init__(self, environment, index):
        ...
        self.expanded = False
        self.numberOfHosts = BlackHoleBrowser.blackHole.data.user.profile.hosts.filter(host__environment=self.environmentObject).count()
        self.update_widget()
...
class HostTree(TreeWidget):
    ...
    def setError(self, message=False):
        ...
        self.error = True
        self._w = urwid.AttrWrap(self.widget, None)
        self.update_w()
