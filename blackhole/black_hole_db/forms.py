...
class StatsByUser(forms.Form):
    user = forms.ModelChoiceField(queryset=User.objects.all().order_by('lastName'),
                                  widget=forms.Select(attrs={'class': 'span3'}), label=_('User:'))
    ...
    to_date = forms.DateField(label=_("To:"),
                              widget=forms.widgets.DateInput(format="%d/%m/%Y", attrs={'class': 'datePicker'}),
                              input_formats=('%d/%m/%Y',))
    statsTypeChoices = (('LOGINS_COUNT', _('Logins Count')),
                        ('USAGE', _('Usage')),
...
class StatsByHost(forms.Form):
    host = forms.ModelChoiceField(queryset=Host.objects.all().order_by('name'),
                                  widget=forms.Select(attrs={'class': 'span3'}), label=_('Host:'))
    from_date = forms.DateField(label=_("From:"),
                                widget=forms.widgets.DateInput(format="%d/%m/%Y", attrs={'class': 'datePicker'}),
                                input_formats=('%d/%m/%Y',))
    to_date = forms.DateField(label=_("To:"),
